...
/// receiver entry, so the boot reads it as locally appended.
///
...
Future<StoredEvent> seedLibVersionEventForTest(
  ...
  DataFormatVersion fromDataFormat = const DataFormatVersion(2, 0),
  DateTime? at,
}) {
  ...
  return backend.transaction((txn) async {
    ...
    if (eventType == LibVersionEvents.initialized) {
      final id = !recordDatabaseId
          ? null
          : databaseId ?? await backend.readOrCreateDatabaseIdTxn(txn);
      data = <String, Object?>{
        'version': version,
