// Runs the drain-wedge scenarios on Sembast. The scenarios' assertions are
// cited on their own tests in test_support/drain_wedge_conformance.dart.
...
class _SembastWedgeDatabase implements QueueTestDatabase {
  _SembastWedgeDatabase(this._db);
  ...
  runDrainWedgeScenarios(
    () async {
      ...
      final db = await newDatabaseFactoryMemory().openDatabase(
        'drain-wedge-$counter.db',
      );
      return _SembastWedgeDatabase(db);
    },
    label: 'sembast',
    retriesConflictingTransactions: false,
    processesShareDatabaseHandle: true,
  );
}
