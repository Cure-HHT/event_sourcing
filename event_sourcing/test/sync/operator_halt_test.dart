// Runs the operator-halt scenarios on Sembast. The scenarios' assertions are
...
class _SembastHaltDatabase implements QueueTestDatabase {
  _SembastHaltDatabase(this._db);
...
void main() {
  var counter = 0;
  runOperatorHaltScenarios(() async {
    counter += 1;
    final db = await newDatabaseFactoryMemory().openDatabase(
      'operator-halt-$counter.db',
    );
    return _SembastHaltDatabase(db);
  }, label: 'sembast');
}
