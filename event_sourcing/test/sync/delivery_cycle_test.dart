// Runs the delivery-cycle scenarios on Sembast. The scenarios' assertions
// are cited on their own tests in
...
class _SembastCycleDatabase implements QueueTestDatabase {
  _SembastCycleDatabase(this._db);
  ...
  runDeliveryCycleScenarios(
    () async {
      ...
      final db = await newDatabaseFactoryMemory().openDatabase(
        'delivery-cycle-$counter.db',
      );
      return _SembastCycleDatabase(db);
    },
    label: 'sembast',
    giveUpInjection: (fail) => (
      failAfterExclusionObtained: fail,
      failEpochBumpWithSerializationFailure: null,
    ),
  );
}
