// Runs the reconfigure scenarios on Sembast. The scenarios' assertions
...
class _SembastReconfigureDatabase implements QueueTestDatabase {
  _SembastReconfigureDatabase(this._db);
...
void main() {
  var counter = 0;
  runReconfigureScenarios(() async {
    counter += 1;
    final db = await newDatabaseFactoryMemory().openDatabase(
      'reconfigure-$counter.db',
    );
    return _SembastReconfigureDatabase(db);
  }, label: 'sembast');
}
