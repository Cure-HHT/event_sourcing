// Runs the destination-wedges view scenarios on Sembast. The scenarios'
// assertions are cited on their own tests in
// test_support/destination_wedges_view_conformance.dart.
...
class _SembastViewDatabase implements QueueTestDatabase {
  _SembastViewDatabase(this._db);
...
void main() {
  var counter = 0;
  runDestinationWedgesViewScenarios(() async {
    counter += 1;
    final db = await newDatabaseFactoryMemory().openDatabase(
      'wedges-view-$counter.db',
    );
    return _SembastViewDatabase(db);
  }, label: 'sembast');
}
