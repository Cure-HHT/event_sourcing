...
**Why closed lists of public operations (assertions B and C).** The public operations write the library's state as part of what they do, and several of them append reserved events: opening an event store records the library version and the registry audit, the registry operations record registrations, recoveries and deletions, the delivery cycle records wedges and resumes, ingest and the receiver endpoint record accepted deliveries, the restore operation records a succession, the chain verification operation, ingest and the delivery cycle record security findings, and the security-context operations record their audits. The chain verification operation only reads the log and appends the security findings its walk records, so it is in both lists. Each appends only the reserved events its own checked operation defines, so a caller cannot choose what they state. The event store's append and transaction operations refuse every reserved entry type, so neither is in the list of assertion C. Every other member that writes or appends a reserved event is private. Both lists are closed: an operation added to either is a change to this requirement. The idempotency store's table is outside the state the storage precondition protects, since action dispatch treats the cache as a pluggable interface; its operations are listed because they write through the library's storage.
...
*End* *Storage barrier* | **Hash**: 35fe949c
