...
L. The library SHALL state, as a precondition of its storage trust boundary, that its delivery guarantees, its views and its security-context records hold only while its persisted state (destination queues, the views it materializes, the records it keeps beside them, such as fill positions, schedules, replay requests, wedge records and the registry check record, and the security context it stores beside each event) changes only through the library's operations.
...
*End* *Destinations* | **Hash**: 8cbecc97
