...
L. The library SHALL state, as a precondition of its storage trust boundary, that its delivery guarantees, its views and its security-context records hold only while its persisted state (destination queues, the views it materializes, the records it keeps beside them, such as fill positions, schedules, replay requests, wedge records, the registry check record, the database identity, the generation records and the view catch-up marks, and the security context it stores beside each event) changes only through the library's operations, and reserved system events are appended only by the library's own operations.
...
*End* *Destinations* | **Hash**: d1b16a7b
