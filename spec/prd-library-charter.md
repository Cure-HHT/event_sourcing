...
- The hash chain from genesis to N is intact: each database's storage chain has no break, and each database's origin chain forks only where one of its skip events lists the fork's successors.
...
- An aggregate for which both sides of a recorded branch point wrote a version is conflicted, with both branches' states, until a reconciliation closes it, and the reconciliation's state supersedes both branches (the substrate could equally serve the latest branch, or merge the two).
