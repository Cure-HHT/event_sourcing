...
H. The library SHALL refuse as malformed, naming the field, a record any of whose provenance entries lacks `database_id` or `library_version`, or carries one that is not a non-empty string: on every ingest entry point before any write, and on every append and read.
...
*End* *The event record as read, stored and sent* | **Hash**: 33428cca
